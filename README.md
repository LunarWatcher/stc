...
| `stc/unix/Process.hpp` | Utility library | Advanced command line execution; supercedes several `Environment.hpp` functions | UNIX only, minus Crapple. Windows support is desired, but won't be implemented for the foreseeable future as I have no applications for it on Windows. |
