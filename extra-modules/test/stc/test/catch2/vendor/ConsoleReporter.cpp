...
class ModConsoleAssertionPrinter {
public:
    ModConsoleAssertionPrinter& operator= (ModConsoleAssertionPrinter const&) = delete;
    ModConsoleAssertionPrinter(ModConsoleAssertionPrinter const&) = delete;
    ModConsoleAssertionPrinter(std::ostream& _stream, AssertionStats const& _stats, ColourImpl* colourImpl_, bool _printInfoMessages)
        : stream(_stream),
        stats(_stats),
...
public:
    explicit ModDuration(double inNanoseconds, Unit units = Unit::Auto)
    ...
    friend auto operator << (std::ostream& os, ModDuration const& duration) -> std::ostream& {
...
public:
    ...
    template<typename T>
    friend ModTablePrinter& operator<< (ModTablePrinter& tp, T const& value) {
        tp.m_oss << value;
        return tp;
    }

    friend ModTablePrinter& operator<< (ModTablePrinter& tp, ColumnBreak) {
    ...
    friend ModTablePrinter& operator<< (ModTablePrinter& tp, RowBreak) {
    ...
    friend ModTablePrinter& operator<<(ModTablePrinter& tp, OutputFlush) {
        tp.m_os << std::flush;
        return tp;
    }
};

ModConsoleReporter::ModConsoleReporter(ReporterConfig&& config):
    StreamingReporterBase( CATCH_MOVE( config ) ),
    m_tablePrinter(Detail::make_unique<ModTablePrinter>(m_stream,
...
ModConsoleReporter::~ModConsoleReporter() = default;

void ModConsoleReporter::noMatchingTestCases( StringRef unmatchedSpec ) {
    m_stream << "No test cases matched '" << unmatchedSpec << "'\n";
}

void ModConsoleReporter::reportInvalidTestSpec( StringRef arg ) {
    m_stream << "Invalid Filter: " << arg << '\n';
}

void ModConsoleReporter::assertionEnded(AssertionStats const& _assertionStats) {
    ...
    ModConsoleAssertionPrinter printer(m_stream, _assertionStats, m_colour.get(), includeResults);
    printer.print();
    m_stream << '\n' << std::flush;
}

void ModConsoleReporter::sectionStarting(SectionInfo const& _sectionInfo) {
    m_tablePrinter->close();
    m_headerPrinted = false;
    StreamingReporterBase::sectionStarting(_sectionInfo);
}
void ModConsoleReporter::sectionEnded(SectionStats const& _sectionStats) {
    m_tablePrinter->close();
...
void ModConsoleReporter::benchmarkPreparing( StringRef name ) {
...
void ModConsoleReporter::benchmarkStarting(BenchmarkInfo const& info) {
    (*m_tablePrinter) << info.samples << ColumnBreak()
        << info.iterations << ColumnBreak();
    if ( !m_config->benchmarkNoAnalysis() ) {
        ( *m_tablePrinter )
            << ModDuration( info.estimatedDuration ) << ColumnBreak();
    }
    ( *m_tablePrinter ) << OutputFlush{};
}
void ModConsoleReporter::benchmarkEnded(BenchmarkStats<> const& stats) {
    if (m_config->benchmarkNoAnalysis())
    {
        (*m_tablePrinter) << ModDuration(stats.mean.point.count()) << ColumnBreak();
    }
    else
    {
        (*m_tablePrinter) << ColumnBreak()
            << ModDuration(stats.mean.point.count()) << ColumnBreak()
            << ModDuration(stats.mean.lower_bound.count()) << ColumnBreak()
            << ModDuration(stats.mean.upper_bound.count()) << ColumnBreak() << ColumnBreak()
            << ModDuration(stats.standardDeviation.point.count()) << ColumnBreak()
            << ModDuration(stats.standardDeviation.lower_bound.count()) << ColumnBreak()
            << ModDuration(stats.standardDeviation.upper_bound.count()) << ColumnBreak() << ColumnBreak() << ColumnBreak() << ColumnBreak() << ColumnBreak();
    }
}

void ModConsoleReporter::benchmarkFailed( StringRef error ) {
...
void ModConsoleReporter::testCaseEnded(TestCaseStats const& _testCaseStats) {
    m_tablePrinter->close();
    StreamingReporterBase::testCaseEnded(_testCaseStats);
    m_headerPrinted = false;
}
void ModConsoleReporter::testRunEnded(TestRunStats const& _testRunStats) {
    printTotalsDivider(_testRunStats.totals);
    printTestRunTotals( m_stream, *m_colour, _testRunStats.totals );
    m_stream << '\n' << std::flush;
    StreamingReporterBase::testRunEnded(_testRunStats);
}
void ModConsoleReporter::testRunStarting(TestRunInfo const& _testRunInfo) {
...
void ModConsoleReporter::lazyPrint() {

    m_tablePrinter->close();
    lazyPrintWithoutClosingBenchmarkTable();
}

void ModConsoleReporter::lazyPrintWithoutClosingBenchmarkTable() {
...
void ModConsoleReporter::lazyPrintRunInfo() {
    m_stream << '\n'
             << lineOfChars( '~' ) << '\n'
...
void ModConsoleReporter::printTestCaseAndSectionHeader() {
...
void ModConsoleReporter::printClosedHeader(std::string const& _name) {
    printOpenHeader(_name);
    m_stream << lineOfChars('.') << '\n';
}
void ModConsoleReporter::printOpenHeader(std::string const& _name) {
    m_stream << lineOfChars('-') << '\n';
    {
        auto guard = m_colour->guardColour( Colour::Headers ).engage( m_stream );
...
void ModConsoleReporter::printHeaderString(std::string const& _string, std::size_t indent) {
...
void ModConsoleReporter::printTotalsDivider(Totals const& totals) {
